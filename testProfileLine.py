...
from   src.geometryLIB             import profileLine
from   src.geometryLIB.profileLine import Side 
...
def main():
    ...
    # profile line 
    A          = [0.2, 0.3, 0.3, 0.3, 0.3, 0.15]
    LEradius   = chord * 2.5e-2 / 2
    TEradius   = 2.5e-2 / 2 
    wedgeAngle = 30
    ...
    A1 = sLine.scale(N=15, camberline=cLine, overwrite=True)
    ...
    pLine1.plot(ax=ax, color='orange', normalized=normalized, pitch=pitch, number=1)
    sLine1.plot(ax=ax, color='c', normalized=normalized, pitch=pitch, number=1)
